...
contract ProxyFactoryUser {
    ...
    function proxyFactory_upgradeImplementationFor(address factory, address proxy, uint256 toVersion, bytes calldata migrationArguments) external {
        IProxyFactory(factory).upgradeImplementationFor(proxy, toVersion, migrationArguments);
    }
    ...
    function try_proxyFactory_upgradeImplementationFor(
        address factory,
        address proxy,
        uint256 toVersion,
        bytes calldata migrationArguments
    ) external returns (bool ok) {
        (ok,) = factory.call(abi.encodeWithSelector(IProxyFactory.upgradeImplementationFor.selector, proxy, toVersion, migrationArguments));
    }
